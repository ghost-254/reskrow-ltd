...
export default function RentPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        const propertiesQuery = query(
          ...
          where("listingType", "in", ["For Rent", "For Lease"]),
  ...
  const PropertyCard = ({ property }: { property: Property }) => (
    <Card className="group hover:shadow-xl transition-all duration-300 border-0 shadow-md overflow-hidden bg-card">
      <div className="relative">
        <Image
          src={property.images?.[0] || "/placeholder.svg?height=250&width=400&query=modern rental property interior"}
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          <Badge className="bg-blue-600 hover:bg-blue-700 text-white">
            ...
            {property.listingType === "For Rent" ? "FOR RENT" : "FOR LEASE"}
          </Badge>
  ...
  const LandCard = ({ land }: { land: Land }) => (
    <Card className="group hover:shadow-xl transition-all duration-300 border-0 shadow-md overflow-hidden bg-card">
      <div className="relative">
        <Image
          src={land.images?.[0] || "/placeholder.svg?height=250&width=400&query=land for lease"}
          alt={land.name || "Land"}
          width={400}
          height={250}
          className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300"
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-muted-foreground">Loading rental properties...</p>
        </div>
      </div>
    )
  }
          ...
          <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 lg:py-32">
            <div className="max-w-4xl mx-auto text-center text-white">
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-6 text-balance">
                Find Your
                <span className="block text-white/90">Perfect Rental</span>
              </h1>
              <p className="text-lg sm:text-xl text-white/80 mb-8 max-w-2xl mx-auto text-pretty">
                Discover exceptional rental properties and lease opportunities across residential, commercial, and
                industrial spaces. Your ideal space is waiting.
              </p>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            ...
            <TabsList className="grid w-full sm:w-auto grid-cols-4 h-auto">
              ...
              <TabsTrigger value="lease" className="text-sm py-2 px-4">
                For Lease
              </TabsTrigger>
          ...
          <TabsContent value="all" className="space-y-8">
            ...
            {lands.length > 0 && (
              <div>
                <h3 className="text-xl font-semibold text-foreground mb-6">Land Leases</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {lands.map((land) => (
          ...
          <TabsContent value="lease">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProperties
                .filter((p) => p.listingType === "For Lease")
                .map((property) => (
                  <PropertyCard key={property.id} property={property} />
                ))}
            </div>
        ...
        {filteredProperties.length === 0 && lands.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-foreground mb-2">No rental properties found</h3>
            <p className="text-muted-foreground mb-6">Try adjusting your search criteria or browse all rentals</p>
            <Button
              onClick={() => {
                setSearchTerm("")
                setPropertyType("all")
